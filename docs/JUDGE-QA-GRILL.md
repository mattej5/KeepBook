...
Three honest parts. (1) The Courier pass landed ~1:30 PM, after freeze, and needed a community 4-bit quant, trimmed memory-safety margins, and a quiet machine — the shipped path is the one verified stable end-to-end. (2) We closed the fairness question ourselves: we ran the identical pan-and-scan strategy back through Ollama (`results_ollama_e4b_panscan.json`, on the repo) — 82.1% at 22.7s vs Courier's 91.5% at 7.1s. Matched technique, Courier still wins both axes; it's in PRD §8 in our own words. (3) The e2b 20/20 is n=5 docs on the two easiest buckets, never the full 32. What panscan really shows: image resolution is a huge lever for both models — which is why preprocessing and region crops are the two interventions that shipped, and why Ollama's careful mode (92.5%) is still the single most accurate config we measured.
...
⚠️ If asked "show me the flag": open View model trace on the doc — the classifier's raw response carries `"handwritten": true` (verified live on a fresh intake at 2:12 PM, which also reproduced the 457-for-437 SSN misread exactly). There's no separate badge in the Review UI; the honest framing is that detection is recorded per-doc in the trace and NO document, handwritten or not, can enter a checklist without full review — the gate is universal, so nothing rides on the badge.
