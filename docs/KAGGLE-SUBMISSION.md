...
The hardest hour was 2 AM. Our provisional eval came back 26/26 on classification and 0 of 94 on fields, for both models at once. Two different models don't usually fail in identical ways, but a harness fails the same way for everyone, so we went looking at our harness instead of our prompts. It turned out our own test-set generator was rendering full-page forms so small that once the vision encoder downscaled them, the field text was illegible. The oversized "W-2" title survived the downscale, which is why classification stayed perfect while every field came back empty. The two models even failed in different ways, and the difference was the same behavior gap the product is built around. `e4b` returned empty strings, in effect admitting it couldn't read the page. `e2b` filled the blanks by presenting the forms' printed box labels as extracted answers, a confident wrong value aimed this time at our own test set. We wrote red tests that pinned the bug, fixed the generator, regenerated all 26 images, and reran everything before we believed a single number.

The rest of the hard parts were about holding discipline on a one-day clock. We kept to a rule we had declared before the bake-off, that no runtime gets named publicly until it passes our kill test, and the record of what passed and what failed lives in PRD §8 of the repo. We rejected two of our own accuracy interventions when they missed the latency or silent-wrong gates, even though both had looked promising. And we froze features just before 1 PM so the last two hours went to rehearsal and to writing up what we could actually prove, instead of more code.
