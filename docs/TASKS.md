...
- [x] **T69 — Client-list CSV import** (agent; PULLED FORWARD ~12:10, shipped before freeze)
  DoD (rescoped by orchestrator for freeze safety): FRONTEND-ONLY — tolerant CSV parser (frontend/js/csv.js, DOM-free/unit-tested: quoted commas, doubled quotes, header-row skip, semicolon/pipe doc lists, blank rows) + preview-then-commit UI in the Add Client form riding the already-verified POST /clients (no new backend surface an hour before freeze); duplicates skipped case-insensitively (vs existing clients AND within-file); bare name-only rows seed expected_docs from the organizer template; partial-failure honesty ("Imported X of Y — [error]").
  Verify: import a CSV → clients render with correct checklists; duplicates skipped, not doubled.
  Evidence: lane commit `dac792b`, merged (suite 69 green, purely additive app.js diff — zero deletions). Agent: parser 8/8 node assertions; live 6-row CSV E2E on :8116 → preview "6 clients found · 2 duplicates will be skipped · 1 blank rows ignored", 4 POSTs created with correct expected_docs, dupes correctly absent. Orchestrator real-browser click-path on :8100 (synthetic File → change event): preview rendered "3 clients found · 1 duplicate will be skipped" (Ruth deduped against seed), Import 2 clicked → GET /clients shows `client_vasquez_priya ['W-2','1099-INT']` + `client_torrey_pines_trust` with the full 9-item organizer template (bare-name seeding). State reseeded after verification.

- [ ] **T71 — Relocatable data directory (backup story)** (agent; POST-DEMO backlog — added Sat ~12:55 per founder)
  DoD: a `KEEPBOOK_DATA_DIR` env var relocating the whole data footprint (state.json, events.jsonl, raws/, uploads/) to any path — including a mounted share on the firm's own file server or NAS. That is the backup answer that preserves the compliance thesis: the firm's own server is not a third-party processor, so durability never reintroduces the DPA problem. First-run setting in the future installer (PRD §7). Until then the honest answer is: one folder (`backend/`), atomic writes (crash-safe by design), covered by Time Machine with zero configuration.
