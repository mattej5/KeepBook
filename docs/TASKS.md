...
Owners: **A** = Andrew, **V** = Vin, **agent** = any coding agent (with the owner reviewing).
...
- [x] **T06 — Team registered; repo public** (V + A)
  Evidence: Vin confirmed registration; `gh repo view higg22-git/KeepBook --json isPrivate` → `false`.

---

## Phase 1 — Backend core (owner A; target ~11:00 AM)

- [ ] **T10 — `backend/model_runtime.py` adapter** (A)
...
- [ ] **T11 — FastAPI endpoints per docs/API.md** (A)
...
- [ ] **T12 — Classification + extraction prompts** (A)
...
## Phase 2 — Eval (owner A + agent; target ~12:30 PM)

- [ ] **T20 — `eval/run_eval.py` per docs/EVAL.md** (A/agent)
...
- [ ] **T21 — Full e4b run over the 26-doc test set** (A/agent)
...
- [ ] **T22 — e2b comparison run** (A/agent)
...
- [ ] **T40 — E2E on the demo Mac, fully local** (V + A)
...
- [ ] **T50 — Three stopwatch dry-runs** (V + A; by 2:00 PM)
...
- [ ] **T53 — Demo logistics** (V + A; by 2:55 PM)
