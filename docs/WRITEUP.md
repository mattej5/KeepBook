...
- **Runtime:** Ollama, verified end-to-end on the demo machine. (The adapter also implements the OpenAI-compatible local-server shape, which let us bake off a second Mac-native runtime, Courier OS, against Ollama through the same adapter on demo day. Its as-shipped build failed image inference on 24GB hardware in the morning; by early afternoon a quant-matched 4-bit MLX build plus a pan-and-scan image strategy passed all three of our pre-declared gates. We then applied the identical pan-and-scan strategy to Ollama to close the fairness gap, and Courier still won on both axes with matched technique, 91.5% fields at 7.1s median against Ollama's 82.1% at 22.7s on the same full eval. Ollama's region-crop careful mode remains the single highest-accuracy configuration we measured, 92.5%, at four times Courier's latency. The full record is in PRD §8. The demo ships Ollama.)
