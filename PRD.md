...
**Sat ~2:30 PM — MATCHED-TECHNIQUE ARM (VERIFIED, closes the fairness caveat):** the identical pan-and-scan packaging was applied to the Ollama path (`OLLAMA_PANSCAN=1`, default OFF so the verified reference stays byte-for-byte; same crops, same prompt hint) and the full 32-doc eval rerun. **Ollama+panscan: 32/32 doc-type, 87/106 fields (82.1%), 7 silent wrongs, 22.7s median, hand gate 7/12 (exactly at gate). Courier+panscan on the same eval: 97/106 (91.5%), 7.1s.** With matched technique Courier wins fields by 9.4 points and latency 3.2× — the remaining gap is engine-level (MLX vision preprocessing/quant vs llama.cpp Q4_K_M), not the adapter. Panscan also helps Ollama (+20 points over its 62.3% single-image fast mode) at +5s/doc. For the record: Ollama's region-crop careful mode is still the single highest-accuracy configuration measured (92.5% at 28.2s, 29-doc set) — Courier+panscan gets within 1 point of it at 4× less latency. Results: `eval/results_ollama_e4b_panscan.json`.
